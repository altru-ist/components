...
const VoltCheckbox = PrimeCheckbox;

const theme = ref<CheckboxPassThroughOptions>({
  root: {
    class: `relative inline-flex select-none w-5 h-5 align-bottom
  ...
  box: `flex justify-center items-center rounded-sm w-5 h-5
        ...
        p-small:w-4 p-small:h-4
        p-large:w-6 p-large:h-6`,
  icon: `absolute top-0 left-0 text-sm w-full h-full transition-none
...
<template>
  ...
  >
    <template #icon="{ checked, indeterminate }">
      <span v-if="checked" class="material-symbols-rounded" :class="theme.icon"
        >check</span
      >
      <span
