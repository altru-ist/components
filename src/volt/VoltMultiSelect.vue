<template>
  ...
  >
    <template #dropdownicon>
      <span class="material-symbols-rounded h-5 w-5">keyboard_arrow_down</span>
    </template>
    <template #loadingicon>
      <span class="material-symbols-rounded h-5 w-5 animate-spin">refresh</span>
    </template>
    <template #filtericon>
      <span class="material-symbols-rounded h-5 w-5 text-surface-400"
        >search</span
      >
    </template>
    <template #clearicon>
      <span
        class="material-symbols-rounded h-5 w-5 text-surface-400 absolute top-1/2 -mt-2 end-10"
        >close</span
      >
    </template>
...
const theme = ref<MultiSelectPassThroughOptions>({
  root: `inline-flex cursor-pointer relative select-none rounded-md p-fluid:flex w-full h-10
  ...
  option: `cursor-pointer font-normal whitespace-nowrap relative overflow-hidden flex items-center gap-2 px-3 py-2
        ...
        p-focus:bg-surface-100 dark:p-focus:bg-surface-800 not-p-disabled:p-focus:text-surface-800 dark:p-focus:text-surface-0
        p-disabled:p-focus:bg-transparent p-disabled:cursor-not-allowed p-disabled:border-surface-300 dark:p-disabled:border-surface-700 p-disabled:text-surface-400 dark:p-disabled:text-surface-400
        transition-colors duration-200`,
  optionLabel: ``,
  pcOptionCheckbox: {
    root: `relative inline-flex select-none w-5 h-5 align-bottom`,
    ...
    box: `flex justify-center items-center rounded-sm w-5 h-5
            ...
            peer-enabled:peer-hover:p-checked:bg-primary-emphasis peer-enabled:peer-hover:p-checked:border-primary-emphasis
            peer-focus-visible:outline-1 peer-focus-visible:outline-offset-2 peer-focus-visible:outline-primary peer-focus-visible:outline 
            p-disabled:bg-surface-800 dark:p-disabled:bg-surface-400 p-disabled:border-surface-300 dark:p-disabled:border-surface-700 p-disabled:text-surface-700 dark:p-disabled:text-surface-400
            shadow-[0_1px_2px_0_rgba(18,18,23,0.05)] transition-colors duration-200`,
    icon: `text-sm w-[0.875rem] h-[0.875rem] transition-none`,
  },
